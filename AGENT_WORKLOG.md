...
### 2026-08-26 - Claude Code (Fable 5, PC) - agentgov/freeze-layering-semantic-tests - semantic/structural invariant tests for the public-surface authority map (freeze layering, PLAN 5.A2); no pin touched

Agent: Claude Code, model `claude-fable-5`, physical desktop account, dispatched from `C:\dev\shared\agent-governance-plan\` (phase-0 PLAN section 5.A2 "immutable artifacts get byte freeze; source gets semantic tests"). TIER: HIGH (public production repo, freeze-adjacent). OUTCOME: one PR that ADDS behaviour-level tests for the source files pinned in `tests/public-surface-adjacency-map/preservation.test.ts` `FROZEN_IDENTITIES`, without removing, moving or re-pinning any entry and without editing `preservation.test.ts` or `renderingBoundary.test.ts`. Owner authorized push and PR creation; merge NOT authorized and not taken. Built in a fresh clone (`core.autocrlf=false` re-checkout after the first checkout smudged every non `-text` file to CRLF and made the preservation test fail on byteLength alone - recorded, not a repo defect), base `origin/main` `cb2f132` (Merge #139).

Task performed: added `tests/public-surface-authority-map/semanticInvariants.test.ts` (27 tests) and `package.json` script `test:semantic-invariants`, appended at the TAIL of `scripts.check` only (the frozen `BASE_PIPELINE` prefix is untouched; `test:adjacency-preservation` 8/8 confirms). The oracle is the byte-pinned `src/data/public-surface-authority-map/last-known-good.json` plus `runtime-manifest.json`; the manifest identity is cross-checked by rebuilding the snapshot id from the artifact's own SHA-256 and the frozen runtime-snapshot file of that name (byte-identical to the artifact), not from the manifest's own id. Seven invariants, each derived from the artifact and checked against the output the pinned modules (`contract.ts`, `d3AuthorityLayout.ts`, `d3AuthorityKeyboardNavigation.ts`, `d3AuthorityViewport.ts`, `runtimeManifestContract.ts`, `byteIdentity.ts`, `fallback.ts`) derive from a candidate map, each with at least one negative fixture (a mutated copy that must fail on that invariant): I1 relation set unchanged; I2 public/private boundary unchanged; I3 navigation semantics unchanged; I4 protected labels present (map and rendered surface `PublicSurfaceAuthorityMap.astro`, full record names, pinned label shortening); I5 forbidden edges absent (raw scan before the contract: self, dangling, unknown type, malformed/duplicate id, non-navigation, omitted-count); I6 accessibility contract (label limits, uniform node size, arrow-key mapping, keyboard band-reachability, renderer `role`/`tabindex`/`aria-*` at source level because the renderer needs a DOM); I7 SHA-256 of the canonicalised derived output (`1d515088...5d9b`) unchanged - a hash of derived output, not of source bytes, so a behaviour-preserving toolchain change reproduces it.

Files changed (2, plus this entry): `tests/public-surface-authority-map/semanticInvariants.test.ts` (new), `package.json` (+1 script, `check` tail append). Commit `99897c0`. NOT changed: `preservation.test.ts`, `renderingBoundary.test.ts` (guard 9), `pnpm-lock.yaml`, dependencies, any file under `src/`.

Tests or build checks run (Windows clone, Node 24.18.0, pnpm 10.34.5): `pnpm run check` ran green through `astro build`, `check:astro`, `check:ts`, `test:metadata-contract`, `test:metadata-verifier-lifecycle`, `wrangler deploy --dry-run`, `test:contracts` 52, `test:authority-layout` 68, `test:authority-viewport` 61, `test:authority-keyboard` 38, `test:runtime` 55, `test:retention`, then stopped at `test:orchestration`'s documented Windows-only `tar: Cannot connect to C:` defect. Every remaining chain step run individually: all exit 0 (`test:adjacency-preservation` 8/8, `test:adjacency-rendering-boundary` 13/13, `test:semantic-invariants` 27/27, ...) except `verify:public-surface-adjacency-map` PSADJ-21 build-determinism, the second documented Windows-only artifact (20/21). Both are pre-existing per the 2026-08-25/26 entries above and were not re-litigated. **The Linux `site-ci` run on the PR is authoritative for readiness.**

Independent review before push: reviewer Codex CLI (`codex-cli 0.149.1`, `model_reasoning_effort=high`), lineage `openai`, mode sequential (stage = full diff + changed files + read-only context + brief; child env built from an explicit allowlist), commit reviewed `99897c0` pre-commit working tree, evidence `C:\dev\shared\agent-governance-plan\versions\20260826-1601-site-freeze-pr\codex-review-r1\` and `...\codex-review-r2\`. Round 1: REJECT (A1 keyboard negative fixture did not exercise the checker; A2 label check accepted changed suffixes of truncated names; B1 manifest id compared with itself; B2 boundary statements and grouping fields checked as subsets only; C1 source-level accessibility check is heuristic). All four A/B remediated (shared `keyboardReachabilityViolations` helper; full-name equality plus `shortenLabel` comparison; independently rebuilt snapshot id; exact set comparisons; three new negative fixtures). Round 2: REJECT on one new A (group-region `aria-label` not in the required renderer attribute set); remediated with one required needle and one negative fixture. Round 2 also confirmed Q1 (no frozen identity, prefix or guard 9 touched), the B1 fix, and I7 determinism by inspection. **The round-2 A fix is coordinator-remediated and reviewer-unverified** - the 2-round cap was reached; the PR is opened with this stated rather than a third round dispatched.

Pre-append inventory gate, run before this write: `git fetch origin --prune`; `origin/main` = `cb2f132` (Merge #139), unchanged since the entry above. `gh pr list --state open` returns zero PRs. `node scripts/check-agent-worklog-governance.mjs` exit 0 (`Rollover line status: review_threshold_reached` - the 4,000-line rollover review is an owner item already in the owner queue, not executed here). The same six non-bot branches carrying `requires_author_or_pr_review=true` under standing 2026-08-06/08-15/08-17 owner rulings are unchanged: `chore/update-download-artifact-20260801` (#106, `merged_via_pr_or_squash` via #107), `rev10-deployment-metadata` (no PR, `merged_via_pr_or_squash` by byte-identical payload), `codex/update-site-from-meta-writing-ecology` (#10, hold), `fix-public-surface-metadata-and-crawler-files` (#1, hold), `claude/p7-1-implementation-plan-7t42ah` (#101, hold), `claude/related-governance-surface` (#127, hold). Nothing new is `completed_pushed_unmerged`, `ambiguous`, or `author_status_unknown`. Gate does not stop.

Unresolved questions (for owner review, REPORTED UNRESOLVED, not blocking this PR): (1) which `FROZEN_IDENTITIES` source pins to retire now that behaviour-level tests exist - candidates with direct coverage: `contract.ts`, `d3AuthorityLayout.ts`, `d3AuthorityKeyboardNavigation.ts`, `d3AuthorityViewport.ts`, `fallback.ts`, `runtimeManifestContract.ts`, `byteIdentity.ts`; NOT proposed on this PR alone: `d3AuthorityRenderer.ts` (source-level attribute check only), `runtimeLoader.ts` (covered by `runtimeLoader.test.ts`, not here), `publicSurfaceAuthorityMap.client.ts`, the `.astro` component and pages (only boundary-statement presence checked). Removing any pin is an owner freeze exception and is outside this PR. (2) Whether the I7 derived-output hash should be re-verified on Node 22 by CI before being relied on (the pin was captured on Node 24; Codex judged the construction deterministic by inspection; the CI run on this PR is the first Node 22 execution). (3) The round-2 A fix is unverified by an independent reviewer (see above).

Risks or assumptions: the tests protect behaviour through the outputs the pinned modules derive for three fixed render situations; mutations to fields no module renders (`evidence_source`, `boundary_references`, `generated_from`) are not covered by I1-I7 and remain covered only by the artifact byte pin and `assertSnapshot`'s internal consistency (Codex Q3). Local Windows results are not the gate; CI is. Merge is owner-reserved and NOT taken here.

### 2026-08-26 - Claude Code (Fable 5, PC) - agentgov/freeze-layering-semantic-tests - follow-up: Guard lifecycle (review_after / sunset_condition) for the semantic-invariants test

Agent: Claude Code (Fable 5), physical desktop, same clone as the entry above (`core.autocrlf=false`). TIER: HIGH (public production repo, PR follow-up). OUTCOME: the guard introduced by PR #140 carries `review_after` and `sunset_condition` in the test file header and in the PR description (PLAN 5.A1 rule; local evidence `agent-governance-plan/versions/20260826-1700-site-prs-sunset/`); pushed; merge left to the owner.

Task: add a short "GUARD LIFECYCLE" comment block at the top of `tests/public-surface-authority-map/semanticInvariants.test.ts`: review_after 2026-11-26; sunset_condition: EITHER the seven source pins with direct coverage (`contract.ts`, `d3AuthorityLayout.ts`, `d3AuthorityKeyboardNavigation.ts`, `d3AuthorityViewport.ts`, `fallback.ts`, `runtimeManifestContract.ts`, `byteIdentity.ts`) are retired by owner ruling and this test becomes the sole protection, OR the test is retired if those pins stay byte-frozen indefinitely. The same line was added to the PR #140 description with `gh pr edit --body-file`. The `AGENTS.md` "Guard lifecycle" subsection that states the rule itself is carried by the sibling PR #141, not duplicated here.

Files changed: `tests/public-surface-authority-map/semanticInvariants.test.ts` (39,950 B -> 40,601 B, 997 LF lines, sha256 prefix `b2aabe4e27fa`; 11 comment lines inserted after the `@ts-nocheck` preamble, no executable line touched; the file stays outside the frozen prefix and no pin is touched), `AGENT_WORKLOG.md` (this entry, tail append only).

Tests or build checks run (Windows clone): `node --test tests/public-surface-authority-map/semanticInvariants.test.ts` 27 pass / 0 fail after the edit; `node scripts/check-agent-worklog-governance.mjs` exit 0 before this append (`Rollover line status: review_threshold_reached`), re-run after the append (result recorded in the local evidence folder and the PR). The Linux `site-ci` run on the pushed head is the readiness authority.

Review provenance: Reviewer interface: Codex CLI (`codex exec`, sandbox workspace-write, reasoning medium, staged payload = diff + changed file + brief, no repo access). Reviewer lineage: openai. Review mode: sequential. Reviewed commit: the head pushed by this entry (comment and governance text only). Review evidence reference: local `agent-governance-plan/versions/20260826-1700-site-prs-sunset/review-140/`; verdict and any single A/B remediation round are recorded there and in the PR, not by rewriting this entry.

Pre-append inventory gate, run at write time (2026-08-26T16:11Z): `git fetch origin --prune`; `origin/main` = `cb2f132` (unchanged since the entry above). `gh pr list --state open` returns exactly one PR that is not this branch: `#141` `agentgov/worklog-prefix-check-and-rule-elevation` (tip `02b69c4` at inventory time, not draft, repository account). Both #140 and #141 are owner-authorized work of the same task (owner authorized push; merge reserved); #141 is listed as author-declared `in_progress`. #141 also appends to `AGENT_WORKLOG.md` in this same round (and edits `AGENTS.md`, `package.json`); whichever of #140 / #141 lands second must tail-re-append its worklog entry onto the landed `main` worklog, re-run the byte-prefix check, and re-check that both new `check` steps sit after the frozen prefix. The six standing non-bot branches carrying `requires_author_or_pr_review=true` are unchanged from the entry above under standing 2026-08-06/08-15/08-17 owner rulings. Gate does not stop.

Unresolved questions (for owner review, not blocking): (1) the sunset condition is stated, not scheduled - nothing in the repository enforces the 2026-11-26 review date; (2) which pins to retire, the Node 22 re-verification of the I7 hash, and the unreviewed round-2 A fix remain open exactly as listed in the entry above.

Risks or assumptions: comment-only change to the test; no invariant, fixture, or pin changed. Merge is an owner decision and is not taken here.

Correction and review outcome for the entry above (same entry, same agent, 2026-08-26): Reviewed commit, exact SHA: `60f4be7844f9b954d05879fa0d7f91115e8d0b9a` (Codex CLI round 1, staged diff of that commit, verdict REJECT: A1 the sunset condition "retired if the pins stay byte-frozen indefinitely" is not decidable at 2026-11-26, and its first branch - pins retired, test becomes sole protection - is a reason to keep the test, not to sunset it; B1 the PR description says `test:semantic-invariants` 23/23 while the file declares 27 tests and 27 pass). Remediation in the commit following this correction: the header now states the sunset as a decidable owner action at or after review_after (retire only if the owner rules the byte pins stay the sole protection), a separate renewal condition (any pin retired -> this test MUST be renewed), and the default at review_after (renew); the PR description carries a test-count correction (27, not 23). The file stays comment-only. The GitHub Copilot review of `48f4bd5` / `60f4be7` (2026-08-26T16:13:18Z, "Needs a closer look") lists four robustness items in the test code from the earlier round (unguarded `rawObject.edges` shapes at line 375, edge-id parsing at line 555, unconditional `probe` layout at line 323, `.find(...)` mutation in negative fixtures at line 811); not touched by this comment-only follow-up, recorded as BACKLOG for the owner. The commit SHA carrying this correction is recorded in the PR and the local evidence folder, not in this byte-preserved entry.
...
Correction and review outcome for the entry above (same entry, same agent, 2026-08-26): Reviewed commit, exact SHA as `AGENTS.md` requires: `a8b2a1040297f64de62e4f759b8aaedbd1817d4f` (Codex CLI round 1, staged diff of that commit, verdict ACCEPT_WITH_FINDINGS: A1 the entry wrote "the head pushed by this entry" instead of the exact SHA - corrected here; B1 the lifecycle sentence did not say who may renew or retire a guard - `AGENTS.md` now states it is an owner decision and the subsection grants no permission; B2 the environmental-note sunset "CI runs on Windows" was too broad - narrowed to both listed failures no longer reproducing, or a Windows CI job becoming a readiness authority). The GitHub Copilot review of `a8b2a10` (2026-08-26T16:18:34Z, "Needs a closer look") concerns `scripts/check-agent-worklog-governance.mjs` (SHA-fetch path versus existing git network configuration; a hardcoded integration-branch label) - code from the earlier round, not touched by this follow-up; recorded as BACKLOG for the owner, not remediated here. The commit carrying this correction and the two `AGENTS.md` wording changes is the new head; its SHA is recorded in the PR and the local evidence folder, not in this byte-preserved entry.

### 2026-08-26 - Claude Code (Fable 5, PC) - agentgov/worklog-prefix-check-and-rule-elevation - integration merge of main (PR #140, 9fa0633) into this branch; worklog tail re-append; check chain re-tailed

Agent: Claude Code (Fable 5), physical desktop, fresh clone with `core.autocrlf=false`, committer identity `Claude Code <noreply@anthropic.com>`. TIER: HIGH (public production repo). OUTCOME: PR #141 mergeable on top of the current `main` with nothing lost from either side, no reviewed commit rewritten, the frozen check-pipeline prefix intact, and `origin/main`'s worklog an exact byte prefix of this file; push authorized by the owner, merge left to the owner.

Task: the owner merged PR #140 first (`main` = `9fa06334a199a519558e31a609e4d9c056495e55`), which made PR #141 (head `3fe89c0acf1c159d4e73e2a0966e1285c467232b`) CONFLICTING on `AGENT_WORKLOG.md` and `package.json`. This is exactly the case the two entries above anticipated ("whichever of #140 / #141 lands second must tail-re-append its worklog entry onto the landed `main` worklog, re-run the byte-prefix check, and re-check that both new `check` steps sit after the frozen prefix"). Resolution is a MERGE COMMIT of `origin/main` into the branch (`git merge --no-ff`), not a rebase, so that the reviewed commits `693809c`, `02b69c4`, `a8b2a10`, `3fe89c0` keep their SHAs for provenance. Merge base `cb2f1329184cd694722e416f1cb579e8789ba025`.

Conflict resolution, both files rebuilt from committed bytes by a script, not by hand-editing conflict markers: `AGENT_WORKLOG.md` = `origin/main`'s file (744,150 B, 4,771 LF, ends with the #140 entry) followed byte-for-byte by this branch's entries in their original order (the 14,162 B / 44 LF that follow the 730,456 B merge-base file in `3fe89c0`; 0 CR, 0 C0/C1 control bytes other than LF, 0 U+FFFD) -> 758,312 B, 4,815 LF, sha256 prefix `3f74b36646ed`; `origin/main`'s file verified as an exact byte prefix of the result before writing and again after re-reading it. `package.json` = `origin/main`'s file (5,278 B, chain ending `test:html-charset && test:semantic-invariants`) with `test:check-pipeline-structure` appended AFTER `test:semantic-invariants` at the tail of `scripts.check`, plus the `test:check-pipeline-structure` script line from this branch -> 5,411 B, 75 LF, sha256 prefix `adf634b11754`. This is the placement the "Frozen Check-Pipeline Prefix" rule this branch adds prescribes: both new steps sit after the twenty-step frozen prefix, neither inserted inside it. All other files (`.gitattributes`, `AGENTS.md`, `scripts/check-agent-worklog-governance.mjs`, `tests/check-pipeline-structure.test.ts`, the three fixtures, and #140's `tests/public-surface-authority-map/semanticInvariants.test.ts`) merged cleanly with both sides taken; `tests/public-surface-adjacency-map/preservation.test.ts` and every `FROZEN_IDENTITIES` path untouched.

Files changed (merge commit `899ff0bc395bc1fcd2f8d4c200e336f354566e0d`, diff against `origin/main`): `.gitattributes`, `AGENTS.md`, `AGENT_WORKLOG.md`, `package.json`, `scripts/check-agent-worklog-governance.mjs`, `tests/check-pipeline-structure.test.ts`, `tests/fixtures/check-pipeline/insertion-2026-08-15.json`, `tests/fixtures/check-pipeline/insertion-e00d6cf-parent.json`, `tests/fixtures/check-pipeline/tail-append.json` - the same nine files PR #141 already carried, no new file. `AGENT_WORKLOG.md` (this entry, tail append only, in the commit after the merge commit).

Tests or build checks run on the merge commit (Windows clone, PowerShell): `pnpm install --frozen-lockfile` OK; `node scripts/check-agent-worklog-governance.mjs` exit 0 with `Append-only (byte prefix at observed integration commit 9fa06334): true` (re-run after this append, result in the local evidence folder and the PR); `pnpm run test:check-pipeline-structure` 6 pass / 0 fail (the positive tail-append case now describes the live chain); `pnpm run test:adjacency-preservation` 8 pass / 0 fail; `pnpm run test:semantic-invariants` (#140's step) 27 pass / 0 fail. A full `pnpm run check` was started from PowerShell; its outcome is recorded in the local evidence folder, subject to the Known-Environmental Failures note (PSADJ-21). The Linux `site-ci` run on the pushed head is the readiness authority.

Review provenance: Reviewer interface: Codex CLI (`codex exec`, sandbox workspace-write, reasoning medium, staged payload = `git diff origin/main...HEAD` of the merge result, the resolved `package.json`, the last 400 lines of this file, a brief; no repo access). Reviewer lineage: openai. Review mode: sequential. Reviewed commit: `899ff0bc395bc1fcd2f8d4c200e336f354566e0d` (the merge commit) together with the worklog-append commit that follows it, whose SHA is recorded in the PR and the local evidence folder, not in this byte-preserved entry. Review evidence reference: local `agent-governance-plan/versions/20260826-1750-site-141-integration-merge/` (BEFORE/AFTER identities of both conflicted files on both sides, the diff, the review text, verdict, any single A/B remediation round, CI and Copilot observations). A fresh GitHub Copilot pull-request review is requested on the pushed head.

Pre-append inventory gate, run at write time (2026-08-26T16:50Z): `git fetch origin --prune`; `origin/main` = `9fa0633` (PR #140 merged 2026-08-26T16:45:14Z). `gh pr list --state open` returns no PR other than this branch's #141. `agentgov/freeze-layering-semantic-tests` is now `merged_via_pr_or_squash` (#140, MERGED). The six standing non-bot branches carrying `requires_author_or_pr_review=true` are unchanged from the entries above (`chore/update-download-artifact-20260801`, `rev10-deployment-metadata`, `codex/update-site-from-meta-writing-ecology`, `fix-public-surface-metadata-and-crawler-files`, `claude/p7-1-implementation-plan-7t42ah`, `claude/related-governance-surface`; standing 2026-08-06/08-15/08-17 owner rulings). Rollover line status `review_threshold_reached` (4,816 lines by the checker's count; owner queue item, not executed here). Gate does not stop. The `origin/main` worklog was verified as an exact byte prefix of this file before and after this append (744,150 B base).

Unresolved questions (for owner review, not blocking): (1) the BACKLOG items the Copilot reviews of `48f4bd5` / `60f4be7` (#140) and `a8b2a10` (#141) raised are unchanged and untouched by this merge; (2) worklog rollover remains a separate authorized task; (3) items (2)-(3) of the `2026-08-26/27` entry above (LF normalisation of `src/pages/ai-readable-knowledge-architecture.md`; the checker's `unknown` ancestry reading) remain open.

Risks or assumptions: no source, test, or pipeline file was hand-edited in this round; the two resolutions are byte compositions of the two parents. The merge commit is a new commit on this branch and the PR will be reviewed on it. Merge is an owner decision and is not taken here.

Correction and review outcome for the entry above (same entry, same agent, 2026-08-26): Reviewed commit, exact SHA as `AGENTS.md` requires: `4e1ac8af704296bea691857d71a6085444c0074b` (the worklog-append commit on top of merge commit `899ff0bc395bc1fcd2f8d4c200e336f354566e0d`; Codex CLI round 1, staged payload = `git diff origin/main...4e1ac8a`, both parents' `AGENT_WORKLOG.md` and `package.json` in full, verdict ACCEPT_WITH_FINDINGS: A1 the entry named only the merge commit as reviewed and deferred the append-commit SHA to the evidence folder - corrected here; questions 1-6 all answered with byte comparisons: `origin/main`'s worklog is an exact byte prefix of the result, the 14,162 B / 44 LF PR #141 tail is intact and in order, `scripts.check` = `main`'s 35 steps plus one tail step, no frozen identity in the diff file list, no loss from either side). `4e1ac8a` was pushed before this correction; the commit carrying this correction is the new head and its SHA is recorded in the PR and the local evidence folder, not in this byte-preserved entry.

### 2026-09-19 - Claude Code (Fable 5.1, Claude Code on the web / remote container) - claude/tender-turing-tniexe - follow-up public surface case (2026-09-19) extending the 2026-08-18 specimen; sibling preserved byte-for-byte

Agent: Claude Code, model `claude-fable-5-1`, Claude Code on the web (isolated Linux container, `pnpm` 10.34.5, Node 22.22.2). Owner instruction sequence this session: (a) "look at the 鍘美藝術節 page" (read-only report, no change); (b) "the event happened, I want to update the data; read the screenshots and the current CNA / PTS reports"; (c) "原頁面可以保留，或是延伸" (the original page may be kept, or extended); (d) two screenshot batches uploaded to the session with the rule "these are reference material; if a figure is ever placed on the page, use our own processed structural or line diagram". Owner-reserved decisions NOT taken here: merge, PR, navigation inclusion, final title / route / wording of the new page, promotion of any candidate concept, and whether the internal link from the new page to the 2026-08-18 page is acceptable.

Task: implement (c) in the most reversible form. The 2026-08-18 specimen is a dated, verify-only page whose boundary statements say the festival was not verified as held; editing those statements would turn a dated specimen into a revised one, which the 2026-08-22 entry already flagged as an owner call even for a rendering repair. So the specimen is left byte-identical to `origin/main` (verified with `git diff --quiet origin/main -- src/pages/artistic-research/public-surface-case/2026-08-18.astro` before commit) and a new dated sibling page is added as the extension.

The change:
- New page `src/pages/artistic-research/public-surface-case/2026-09-19.astro` - standalone `<!doctype html>` document in the same idiom as its two siblings (imports `global.css`, hand-written head, `<meta name="robots" content="noindex, nofollow">`, same class vocabulary; one new `.trajectory` / `.step` style pair for a dated definition list, modelled on `.reading-row`). Title `Semantic Casting, One Month On`; case date 19 September 2026; two evidence windows (platform field 18 August - 5 September 2026; institutional 18-19 September 2026, UTC+8). Genre line states `follow-up ≠ revision; later evidence ≠ earlier error`. Movements: 01 what the earlier specimen held open (its non-verification statements were true at their date and are left as written); 02 source boundary, now four layers (August news snapshots; platform-field material; institutional reporting; MWE reading) with the Observed / Research reading / Limit rubric and a Traditional Chinese gloss carrying `lang="zh-Hant"`; 03 a dated trajectory of the platform-field layer (2026-08-18, 08-21, 08-25 to 29, 08-30, 09-04, and 09-18 to 19 institutional), written abstractly - no account handle, personal name, troupe name, performer name, engagement count, or cash figure is taken from the screenshots; the research-reading row says explicitly that whether the trajectory establishes the candidate `Irreversible Cost Threshold` as a property of the case "is not decided here. The candidate stays a candidate"; 04 the field's own boundary statements (21 August: one commissioned company speaks for the event; 30 August: "not a public trial of any person", official notices versus forwarded / derivative content), read as the field managing the cost of its own readability, offered as an observation about one field only; Limits (`not-list`, seven lines); Relation to MWE - the same three corpus links as the 2026-08-18 page plus one internal link to that page, and the seven candidate concepts carried over with status unchanged; no new candidate concept introduced, no new corpus relation added; Source snapshots (three institutional items, one platform-field descriptor, one internal link) plus a folded bilingual preservation note; one-sentence formulation and footer. Every preformatted block is a `<pre>` (the `compressHTML` lesson of the 2026-08-22 entry), so line structure survives the build.
- `scripts/lib/indexing-discovery-contract.mjs` - exact path `"/artistic-research/public-surface-case/2026-09-19/"` added to `SITEMAP_EXCLUDED_PATHS` immediately after its 2026-08-22 sibling (+2 lines, 0 deletions). Required, not optional: without it `verify:indexing-discovery-build` would report the route as an unexpected sitemap entry, as the 2026-08-22 entry measured.
- `tests/indexing-discovery.test.ts` - one new test (`public surface case 2026-09-19: standalone noindex follow-up, exact-path excluded, sibling untouched`): robots meta present, no BaseLayout / canonical, exact route excluded and not sitemap-eligible, adjacent date 2026-09-20 NOT excluded (exact-path proof), the page links to the 2026-08-18 route, and the 2026-08-18 sibling still declares noindex and is still excluded.

Sources and how they were read. Institutional layer: CNA 2026-09-18 `202609180094` (police traffic control / five-evening run), CNA 2026-09-18 `202609180288` (opening night, first performance), Focus Taiwan 2026-09-19 `culture/202609190006` (CNA English service). The container's egress proxy blocks `cna.com.tw`, `news.pts.org.tw`, `focustaiwan.tw`, `taipeitimes.com`, `drive.google.com`; article bodies were therefore NOT read here - headlines, dates and URLs come from the outlets' own listings as returned by web search, and the page says so ("Article bodies are not the basis of any claim on this page"). No PTS report on the festival was found in three domain-limited searches of `news.pts.org.tw`; none is cited. A Taipei Times report exists (`2026/09/19/2003864540`) and is not cited (agency credit unverified). Platform-field layer: 45 dated screenshots (29 + 16, two zip uploads; Threads and one SETN article read through LINE; 2026-08-18 to 2026-09-05) read in full in the container as reference material; not committed to the repository, not reproduced on the page, and the container copies are discarded with the session. Numbers circulating on platforms and in other outlets (attendance, budget, banquet tables, cash distributed) vary between sources and are not adopted anywhere on the page.

Files changed (4): `src/pages/artistic-research/public-surface-case/2026-09-19.astro` (new), `scripts/lib/indexing-discovery-contract.mjs` (+2), `tests/indexing-discovery.test.ts` (+1 test), `AGENT_WORKLOG.md` (this entry, tail append only). NOT changed: `src/pages/artistic-research/public-surface-case/2026-08-18.astro` (byte-identical to `origin/main`), `2026-08-22.astro`, `astro.config.mjs`, `package.json`, `wrangler.json`, navigation, `artistic-research.md`, any `public/` asset.

Tests or build checks run (Linux container): `pnpm install --frozen-lockfile` OK; `astro build` Complete, new route absent from every generated sitemap XML; `check:astro` 0 errors / 0 warnings / 6 hints; `check:ts` exit 0; `test:indexing-discovery` 235 tests, 234 pass, 0 fail, 1 skipped (the pre-existing chmod test; includes the new test); `test:metadata-contract` 27/27; `test:metadata-verifier-lifecycle` 5/5; `verify:indexing-discovery-build` 161/161; `verify:metadata-build` 1129/1129; `verify:public-surface-map` 21/21; `test:html-charset` 16/16; `test:semantic-invariants` 27/27; `test:check-pipeline-structure` 6/6; `test:adjacency-preservation` 8/8; and every remaining step of `scripts.check` run individually in chain order - `test:contracts` 52, `test:authority-layout` 68, `test:authority-viewport` 61, `test:authority-keyboard` 38, `test:runtime` 55, `test:retention` 16, `test:orchestration` 29, `test:workflow` 42, `test:semantic-flow` 21, `test:security-resilience` 128, `test:human-governed` 30, the ten remaining adjacency suites (58 / 18 / 23 / 39 / 10 / 10 / 13 / 56 / 22 / 60), `verify:public-surface-adjacency-map` 21/21 - all 0 fail. NOT run: `wrangler deploy --dry-run` (no Cloudflare credentials in the container and `workers.cloudflare.com` is blocked by the egress proxy), so `pnpm run check` was not executed end-to-end as one chain; the Linux `site-ci` run on the pushed head is the readiness authority. Rendered check: `astro dev` + headless Chromium at 1280 px and 390 px - HTTP 200, no horizontal overflow at either width, the five preformatted blocks keep their authored line structure (6 / 3 / 4 / 3 / 7 lines), the `<details>` opens, all links present; the 2026-08-18 page was rendered the same way at the start of the session (its five blocks 6 / 3 / 3 / 3 / 5 lines, confirming the 2026-08-22 repair is live). Symbol hygiene: the one touched human-facing file contains no literal ASCII not-equal marker (`grep -c '!=' ... -> 0`); the two touched code files legitimately contain `!=` / `!==` operators, preserved. `.gitattributes` LF pins apply; the new file is LF.

Review provenance (boundary-sensitive work; no independent review has been performed before this push): Reviewer interface: none. Reviewer lineage: not applicable. Review mode: not applicable. Reviewed commit: none. Review evidence reference: none. The owner may request a Codex or Copilot review of the pushed head; any such review and its verdict are to be recorded in a later entry, not by rewriting this one.

Pre-append inventory gate, run before this write (2026-09-19T13:3xZ): `git fetch origin --prune`; `origin/main` = `6fc0f4ed593ff0d744d78408650ae44218d57f85` (Merge PR #141, 2026-08-27), unchanged since the last entries above, so the standing owner rulings need no reconfirmation. PR state obtained through the GitHub MCP tools (the governance script itself reports `pr_state_unavailable (gh_absent)`; the two readings are recorded separately, as `AGENTS.md` asks). Open PRs: exactly three, all Dependabot - #142 `dependabot/npm_and_yarn/cloudflare-wrangler-92253c50f0` (cloudflare-wrangler group, 2 updates), #144 `dependabot/npm_and_yarn/dev-tooling-ba30eb769d` (fast-xml-parser 5.11.0 -> 5.11.1), #145 `dependabot/npm_and_yarn/astro-df8e7da9b8` (astro group across 1 directory, 2 updates; supersedes #143, closed unmerged 2026-09-07). Listed as the dependency queue; this task does not concern dependency integration, so they do not gate it. No non-bot open PR. Remote non-bot branches whose tip is not an ancestor of `origin/main`: the six standing branches carrying the 2026-08-06 / 08-15 / 08-17 owner rulings (`chore/update-download-artifact-20260801`, `rev10-deployment-metadata`, `codex/update-site-from-meta-writing-ecology`, `fix-public-surface-metadata-and-crawler-files`, `claude/p7-1-implementation-plan-7t42ah`, `claude/related-governance-surface`) and the two same-stream P7 branches (`claude/p7-0-rendering-boundary-guards-7nuss5`, `claude/p7-1-radial-constellation`) - author-declared `in_progress` / `hold` / `merged_via_pr_or_squash` per those rulings, listed and not blocking. Every other remote branch is an ancestor of `origin/main`. No branch classified `completed_pushed_unmerged`, `ambiguous`, or `author_status_unknown`; gate does not stop. `node scripts/check-agent-worklog-governance.mjs` before this append: exit 0, `Append-only (byte prefix at observed integration commit 6fc0f4ed): true`, 4,838 lines by the checker's count, rollover line status `review_threshold_reached` (owner queue item, not executed here); re-run after this append with the result recorded in the commit message. The inventory is advisory only and authorizes no merge, publication, or deployment.

Unresolved questions (for owner review, not blocking): (1) title (`Semantic Casting, One Month On`), route date (`2026-09-19`, the reading date) and all conceptual wording are defaults chosen by the agent under instruction (c) and are owner-adjustable; Movement 04 in particular is an authored reading and should be read as a draft. (2) Whether the internal link from the new page to the 2026-08-18 page (and its listing under "Existing surfaces (navigation-only)") is acceptable; the 2026-08-22 sibling carries no such link. Neither page is linked from site navigation or `artistic-research.md`. (3) No PTS report was found; if the owner holds a PTS URL it can be added to the Source snapshots block as a fourth institutional item. (4) The three institutional URLs were not fetched from this container (egress blocked); the owner may wish to open them once before merge. (5) Whether a structural diagram of the four-layer boundary or the dated trajectory should be drawn for the page (inline SVG, series-made, per instruction (d)); none was added in this round. (6) `AGENT_WORKLOG.md` remains past the 4,000-line review threshold; rollover is a separate authorized task. (7) No PR was opened (not requested); push to `claude/tender-turing-tniexe` only.

Risks or assumptions: the new route is noindex and sitemap-excluded but, once merged and deployed, reachable and guessable ("not linked" makes it a deployed-but-unlisted specimen, as the 2026-08-18 entry noted). The page names the festival only inside the two CNA headlines in the Source snapshots block and in the Focus Taiwan headline; the prose refers to "the festival named in the earlier specimen's reconstruction keywords". No living person is named; no organizer, troupe, performer, designer, company, or venue is named; the 2026-08-30 statement is paraphrased, not quoted. The screenshot-derived trajectory is presented as platform-field material reconstructed by one observer at irregular intervals, and the page says a gap between dates is a gap in observation. No candidate concept is promoted; no new relation is introduced; the 2026-08-18 page is not modified. Merge, PR, navigation, naming and publication remain owner decisions and are not taken here.

### 2026-09-19 - Claude Code (Fable 5.1, Claude Code on the web / remote container) - claude/tender-turing-tniexe - follow-up: the 2026-09-19 page's Movement 02 gloss rewritten in English on owner instruction

Agent: Claude Code, model `claude-fable-5-1`, same container and session as the entry above. Owner instruction (verbatim scope): "這個用英文就好", quoting the Movement 02 gloss block `延伸說明（中文簡述）` of the new page. Owner-supplied replacement text: none; the owner asked for the same content in English.

The change: in `src/pages/artistic-research/public-surface-case/2026-09-19.astro`, the `.gloss` block in Movement 02 is now `Extension note (summary)` with the two paragraphs rendered in English (same content: extension not revision; the earlier page's non-verification statements kept as written; two added layers, platform field 18 August - 5 September from author-held screenshots not reproduced, institutional reporting from 18 September; institutional reporting confirms only news circulation and does not retroactively verify the platform layer; no verification of attendance, budget or ticketing figures; no candidate concept promoted). The `lang="zh-Hant"` attribute on that block is removed with the Chinese text. No other block changed; the Traditional Chinese paragraph inside the folded "Preservation note" remains as before because the instruction named only the gloss.

Files changed (2): `src/pages/artistic-research/public-surface-case/2026-09-19.astro` (one block), `AGENT_WORKLOG.md` (this entry, tail append only). `2026-08-18.astro` re-verified byte-identical to `origin/main`.

Tests or build checks run (Linux container): `astro build` Complete; `check:astro` 0 errors / 0 warnings; `test:indexing-discovery` 234 pass / 0 fail / 1 skipped; `verify:indexing-discovery-build` 161/161. Symbol hygiene: no literal ASCII not-equal marker in the touched page. Standalone preview copies of both pages (global.css inlined, internal links rewritten to local files) were rebuilt and delivered to the owner in the session; they are preview artifacts outside the repository, not deployed pages. `node scripts/check-agent-worklog-governance.mjs` re-run after this append (result in the commit message).

Pre-append inventory gate: `origin/main` unchanged (`6fc0f4ed`) and no branch or PR state change since the entry above, so the inventory above stands and is not re-derived. Gate does not stop.
...
### 2026-09-19 - Claude Code (Fable 5.1, Claude Code on the web / remote container) - claude/tender-turing-tniexe - follow-up: two movements added to the 2026-09-19 page (speed / cohesion; rapid cultural translation), one owner-introduced case-local formulation, two sources, an owner correction to the official-versus-public reading

Agent: Claude Code, model `claude-fable-5-1`, same container and session as the three entries above. Owner instructions this round, in sequence: (a) the reading the owner wants the page to hold is that a self-organized effort of this size was planned in weeks for the declared purpose of taking one party's side and was actually held; (b) the collective subject must NOT be a people or a place ("不能講台灣"); frame it instead as cohesion forming rapidly under a pluralistic society with democratic conditions; (c) the use of glove puppetry, Taiwanese opera and a popular singer's theme-matched songs is "a kind of rapid cultural translation" (文化快速轉譯); (d) the singer's three songs and the full lyric of one were supplied by the owner from the performance; (e) decisions: do not name the singer; describe songs by subject only; add the two movements; add the sources; the owner asked whether the translation argument is correct; (f) correction mid-implementation: the public genuinely read the event as being about the dispute; the organizers' "no public trial" statement is a line kept for neutrality and legal exposure, not a description of how the event was read.

Assessment given to the owner on (e), recorded here because it shaped the text: the rapid-cultural-translation argument holds for this field, with three limits written into the page - what was translated officially is the template while the public reading stayed literal (revised per (f)); the speed rides on a pre-existing production system (bookable temple-festival troupes, hireable stages, a fixed ritual opening, banquet caterers), not only on pre-compressed cultural material; and there is no reception measurement and no baseline. `Rapid Cultural Translation` is distinct from the 2026-08-18 candidate `Semantic Casting` (a reading operation) as a production operation; the page co-lists them and states no relation.

The change, all in `src/pages/artistic-research/public-surface-case/2026-09-19.astro`:
- New Movement 04 `Speed, scale, and how fast cohesion formed`: rows Speed (17 days to the warm-up, 31 days to opening night, measured between dated screenshots and dated headlines), Scale, Stated motive (the field's self-description: solidarity with one party to a private dispute, "getting even on that party's behalf", participation framed as local belonging; recorded, not adopted), Research reading (cohesion forming in days "in a pluralistic society with open public discourse and democratic latitude for assembly and expression", offered as background conditions "not as a property of any people or place"), Limit (conditions named not measured; motive self-described; no baseline). Four distinctions.
- New Movement 05 `Rapid cultural translation: the repertoire did the work`, with the candidate line `case-local formulation introduced on this page / not a Registry entry: Rapid Cultural Translation`: rows Forms, Repertoire (every production an existing faithless-husband or wronged-woman story; "selected, not composed"), The song layer (the singer unnamed; three songs described by subject - a 1990s puppet-theatre character theme, a wandering-heroine song, a betrayed lover's lament to a faithless partner; the third requested in advance by the public before the appearance was announced; set list attributed to the author's observation), The field's own account (the organizers' 31 August framing of puppetry and popular song as two forms carrying the same language and feeling), Research reading (pre-compressed cultural material AND a pre-existing production system; translation running both ways through the singer's catalogue and the public's advance choice; official translation = template versus public translation = literal, "both readings were present at once, and the public one was the louder"), Limit (set list is author observation; institutional layer establishes only that the singer performed alongside the puppet play and that one song was requested in advance; retitling not verified; reception not measured; lyrics not reproduced). Four distinctions including `the official translation ≠ the public translation`.
- Former Movement 04 renumbered 06. Its Research reading now adds that, read against the public layer, the 30 August statement "functions as a line the official layer maintains for neutrality and for its own legal exposure, not as a description of how the event was read", marked as the author's reading; its Limit now states what IS observable (posts and headlines went on reading the event as being about the dispute after the statement) and what is not (private belief; the organizers' reasons). One distinction added: `an official boundary ≠ the public reading`.
- Header framing sentence, meta description, the flow block (one line added), the 2026-09-18/19 trajectory row (singer alongside the puppet play; television report), the Limits block (one `not` line: "not a review of any performance or song, and not a reproduction of any lyric"; "performer" added to the moral-judgment line; the evidence-count note updated), the Relation section (lead now states one formulation introduced on this page; candidate column retitled `Candidate concepts (not Registry)` with the eighth item `Rapid Cultural Translation - introduced on this page, 2026-09-19`), the Source snapshots (lead sentence; three rows added: CTS 2026-09-19 `news.cts.com.tw/cts/society/202609/202609193080480` television report of the singer and the puppet play; LTN entertainment desk `ent.ltn.com.tw/news/breakingnews/5558626`, dated "Before 18 September 2026 (publication timestamp not retrieved)", cited for the advance song request and marked as a pre-event report of a platform request; `Set list` row, author observation, lyrics not reproduced), the one-sentence formulation and boundary note. One CSS rule added: `.source a{overflow-wrap:anywhere}`, because the CTS URL has no break opportunity and produced a 14 px horizontal overflow at 390 px in the rendered check; after the rule, 1280 / 390 / 360 px all measure scrollWidth = clientWidth.
- Language rule kept: a character-class scan finds CJK text on exactly four lines, all news headlines in the Source snapshots block (CNA x2, CTS, LTN). Both new headlines contain the singer's name and the LTN headline carries tabloid wording; they are retained verbatim as the series' source-snapshot convention requires, while the prose names no one. The owner may drop the LTN row; the advance-request claim would then rest on author observation only.
- NOT on the page: the singer's name in prose, any song title in any language, any lyric (the owner-supplied lyric is copyrighted and was used only to confirm the song's subject), any figure from platforms or other outlets.

Files changed (2): `src/pages/artistic-research/public-surface-case/2026-09-19.astro`, `AGENT_WORKLOG.md` (this entry, tail append only). `2026-08-18.astro` re-verified byte-identical to `origin/main`. No test, contract, config, layout, style file or navigation changed; the existing 2026-09-19 test still passes unchanged.

Tests or build checks run (Linux container): `astro build` Complete; `check:astro` 0 errors / 0 warnings; `test:indexing-discovery` 234 pass / 0 fail / 1 skipped; `verify:indexing-discovery-build` 161/161; `verify:metadata-build` exit 0, no FAIL line. Rendered check on the standalone preview in headless Chromium at 1280 / 390 / 360 px: no horizontal overflow, seven preformatted blocks keep their line structure (7 / 3 / 4 / 4 / 4 / 4 / 8), ten `h2` headings. Symbol hygiene: no literal ASCII not-equal marker in the touched page. Web search (the only outbound path that works from this container) located the CTS and LTN items and confirmed the 1990s puppet-theatre origin of the first song; article bodies were not read. `node scripts/check-agent-worklog-governance.mjs` re-run after this append (result in the commit message).

Review provenance: unchanged from the first 2026-09-19 entry - no independent review has been performed before this push; Reviewer interface none; lineage not applicable; mode not applicable; reviewed commit none; evidence reference none.

Pre-append inventory gate: `origin/main` unchanged (`6fc0f4ed`) and no branch or PR state change since the entries above; the inventory of the first 2026-09-19 entry stands. Gate does not stop.

Unresolved questions (for owner review, not blocking): (1) whether `Rapid Cultural Translation` stays as an owner-introduced case-local formulation on this page or is later moved, renamed or withdrawn - a Registry decision this page does not make; (2) whether the LTN row stays, given the name and wording in its headline; (3) the LTN publication timestamp, not retrievable from this container; (4) the set list rests on the owner's observation and a search summary attributes a different song to the opening - the page therefore claims only what the institutional layer supports and marks the set list as author observation; (5) the wording "pluralistic society with open public discourse and democratic latitude for assembly and expression" is the agent's rendering of the owner's "多元文化社會民主背景" and is owner-adjustable; (6) items (1)-(7) of the first 2026-09-19 entry are unchanged.

Risks or assumptions: Movements 04 and 05 and the revised Movement 06 are authored readings drafted under the owner's direction and remain drafts until the owner approves them; every generalization is fenced to one field and explicitly not to a people or a place. The page reports the field's stated motive and the public's literal reading as observed surfaces and adopts neither. No living person is named in prose. Merge, PR, navigation and publication remain owner decisions and are not taken here.

### 2026-09-19 - Claude Code (Fable 5.1, Claude Code on the web / remote container) - claude/tender-turing-tniexe - follow-up: secondary-compilation layer extracted into the 2026-09-19 page; LTN entertainment source removed on owner instruction

Agent: Claude Code, model `claude-fable-5-1`, same container and session as the four entries above. Owner instructions this round: (a) an AI-assisted deep-research report on the festival (Markdown, ~286 lines, dated to the evening of 19 September 2026) was uploaded to the session with "補充 deep research 的資料，你抽我們需要的東西出來，符合我們的論述" - extract what serves the page's argument; (b) mid-implementation: "去掉那個痛扁渣男的新聞" - remove the LTN entertainment-desk source added in the entry above.

What was extracted from the report, and how it is labelled. The report is treated as a fifth material layer, `secondary compilation`: its citations point to the festival's official site and ticketing page, city traffic notices, CNA and other outlets, and cultural-institution material, none of which is retrievable from this container, so it is read for convergence with the other layers and for order of magnitude, never as verification. Four kinds of content were taken; everything else was left out.
- Trajectory: one new `compiled` step, 2026-09-09 to 12 (a second warm-up performance in another district; banquet ticketing; the first night's 250 tables reported sold in about half a minute; all 750 reported sold out), marked "Not verified here". Movement 03's lead now says one step comes from the compilation.
- Movement 04: a new row `Scale, as compiled` (about five thousand on opening night as an organizer estimate; several hundred at each warm-up; 250 / 750 tables; a main budget of roughly ten million NTD almost entirely from one private funder; over six million NTD raised separately for about three hundred thousand items from sixteen sheltered workshops and welfare groups; a core team grown from five or six to twenty-six; the venue moved from a narrow street near the site of the dispute to a public parking lot), explicitly "orders of magnitude, not verified here"; and a convergence sentence in the Research reading: the compilation's own synthesis that platforms turned private anger into a public issue within days, private capital bypassed institutional process, and the existing cultural structure supplied the meaning, plus its own warning against reading the field as proof of a national character, which the page says it shares.
- Movement 05: a new row `As compiled` carrying the compilation's reading (citing the national traditional-arts centre and the national history museum) that the festival re-activated two existing cultural technologies - an opera form raised on temple stages that has repeatedly absorbed new media, and a banquet form whose function was collective preparation and co-eating - rather than pairing two local ingredients.
- Movement 06: a new row `As compiled` listing the official layer's later boundaries as the compilation reports them (no rumour, harassment or bullying; a four-part rule keeping political figures off the stage and party material out of the controlled area; a volunteer legal advisor; the venue move) and the compilation's reading that neutrality here is a rule of on-site governance, not an absence of political meaning - recorded as converging with the page's own reading from a different direction.
- Movement 02 retitled "three layers, now five" with the compilation added to the layer list; its Limit row now says compiled figures are marked, held as orders of magnitude and not adopted as verified. Limits block: one `not` line added ("not an endorsement of any compilation's figures, forecasts, valuations, or recommendations"); evidence-count note updated. Source snapshots: one `Compilation` row (secondary layer, not reproduced, no name taken from it).
- NOT taken from the report: its academic frames (connective action, resource bricolage, festival-identity survey models), its sustainability scenarios and strategy recommendations, its financial inferences, the gender-framed legal-consultation item, every personal name, and its Mermaid process chart. The report file is reference material outside the repository and is not committed.

LTN removal (b): the `LTN` source row (`ent.ltn.com.tw/news/breakingnews/5558626`, headline with tabloid wording and the singer's name) is removed. The advance-request claim for one song therefore no longer has an institutional anchor and is rewritten as the author's observation of the platform field, in both the song-layer row and the Movement 05 Limit; the evidence-count note drops "one pre-event entertainment report". A scan finds CJK text on exactly three lines, all news headlines (CNA x2, CTS).

Files changed (2): `src/pages/artistic-research/public-surface-case/2026-09-19.astro`, `AGENT_WORKLOG.md` (this entry, tail append only). `2026-08-18.astro` re-verified byte-identical to `origin/main`. No test, contract, config, layout, style file or navigation changed.

Tests or build checks run (Linux container): `astro build` Complete; `check:astro` 0 errors / 0 warnings; `test:indexing-discovery` 234 pass / 0 fail / 1 skipped; `verify:indexing-discovery-build` 161/161; `verify:metadata-build` exit 0, no FAIL line. Rendered check on the standalone preview at 1280 / 390 / 360 px: no horizontal overflow; seven preformatted blocks keep their line structure (7 / 3 / 5 / 4 / 4 / 4 / 9); eight source rows; seven trajectory steps. Symbol hygiene: no literal ASCII not-equal marker in the touched page. `node scripts/check-agent-worklog-governance.mjs` re-run after this append (result in the commit message).

Review provenance: unchanged - no independent review has been performed before this push; Reviewer interface none; lineage not applicable; mode not applicable; reviewed commit none; evidence reference none.

Pre-append inventory gate: `origin/main` unchanged (`6fc0f4ed`) and no branch or PR state change since the entries above; the inventory of the first 2026-09-19 entry stands. Gate does not stop.

Unresolved questions (for owner review, not blocking): (1) whether an AI-assisted compilation should remain a cited layer on a public MWE page at all - it is labelled as secondary and non-verifying, but the owner may prefer to fold its convergent points into the author's reading and drop the source row; (2) the compiled figures are now on the page as orders of magnitude, which reverses the earlier "figures not adopted" stance for that one labelled row; the owner may prefer to keep only the non-numeric items; (3) items (1), (3)-(6) of the entry above, and (1)-(7) of the first 2026-09-19 entry, are unchanged.

Risks or assumptions: every compiled item is attributed to the compilation and fenced as unverified; no name, forecast or recommendation from it appears; the page still names no living person in prose. Merge, PR, navigation and publication remain owner decisions and are not taken here.

### 2026-09-19 - Claude Code (Fable 5.1, Claude Code on the web / remote container) - claude/tender-turing-tniexe - Public Surface Case block added to artistic-research.md on owner ruling (2026-08-18 and 2026-09-19 linked; 2026-08-22 not linked)

Agent: Claude Code, model `claude-fable-5-1`, same container and session as the five entries above. Owner ruling this round, verbatim: "藝術研究可以，本身頁面也可以，你開始處理" - the drafted Artistic Research block is approved, the 2026-09-19 page as pushed is approved, proceed. This ruling supersedes the 2026-08-18 owner decision recorded in the `claude/taiwan-news-webpage-review-ok9oxb` entry ("noindex, standalone, and NOT linked from navigation or artistic-research.md") for the two pages named below, and answers the "formal linking is deferred until the owner decides how this series is navigated" note of the 2026-08-22 entry for those two pages only. Top navigation is unchanged and was not authorized.

The assessment that preceded the ruling (given to the owner in the session, recorded here because it fixed the shape of the change): three options were laid out - (1) a sub-block under the existing `## Selected Public Reading` heading, following the Public Slice precedent of one paragraph plus one `Enter ...` link per page; (2) a series index page at `/artistic-research/public-surface-case/`, noindex (cheap) or indexable (which would move the `INDEXABLE_COUNT` of 42 in `verify-metadata-build` and the "42 indexable, 2 noindex previews" assertion in the adjacency-map metadata test, plus the Package D registry); (3) status quo. Option 1 was recommended for a series of two or three pages, with option 2 deferred until the series is longer. The owner chose option 1.

The change: `src/pages/artistic-research.md`, +10 lines, 0 deletions, inserted after the `2026-08-07` Public Slice link and before `## Exhibition or Presentation Version`: a `### Public Surface Case` sub-heading; one genre paragraph ("reads an external public-discourse field as a bounded, dated, verify-only specimen. Its material is external observation, not repository evidence. A dated specimen is preserved as written; a later reading is added as a separate page rather than as a revision."); one paragraph describing the 2026 pair (first reading at the moment a template was cast; second a month later, recording how fast cohesion formed and how the template was translated into an existing performance repertoire; "Both pages are noindex and are listed here only."); and two links, `Enter Public Surface Case — 18 August 2026` and `Follow-up — 19 September 2026`. The genre sentence deliberately differs from the Public Slice sentence above it, so that repository-evidence slices and external-observation cases do not read as one kind. No existing line of the file was altered, reordered or removed. The 2026-08-22 case is NOT linked: it is second-order and cites repository evidence, so it would need its own genre sentence, which the owner has not ruled on.

What the link changes and does not change: both linked pages remain `noindex, nofollow`, remain in `SITEMAP_EXCLUDED_PATHS`, and remain absent from every generated sitemap; the indexable route count stays 42; no test pins `artistic-research.md` and none was changed; `artistic-research.md` is not in `FROZEN_IDENTITIES`. The pages become discoverable by readers of the Artistic Research page and by crawlers following its links; noindex still keeps them out of search results. Neither case page was modified in this round; `2026-08-18.astro` re-verified byte-identical to `origin/main`.

Files changed (2): `src/pages/artistic-research.md`, `AGENT_WORKLOG.md` (this entry, tail append only).

Tests or build checks run (Linux container): `astro build` Complete; `check:astro` 0 errors / 0 warnings; `check:ts` clean; every step of `scripts.check` run individually in chain order with 0 failures - `test:metadata-contract` 27, `test:metadata-verifier-lifecycle` 5, `test:contracts` 52, `test:authority-layout` 68, `test:authority-viewport` 61, `test:authority-keyboard` 38, `test:runtime` 55, `test:retention` 16, `test:orchestration` 29, `test:workflow` 42, `test:semantic-flow` 21, `test:security-resilience` 128, `test:indexing-discovery` 234 pass / 1 skipped, `verify:public-surface-map` 21/21, `verify:indexing-discovery-build` 161/161, `verify:metadata-build` 1129/1129, `test:human-governed` 30, the eleven adjacency suites (58 / 18 / 23 / 39 / 10 / 10 / 8 / 13 / 56 / 22 / 60), `verify:public-surface-adjacency-map` 21/21, `test:html-charset` 16, `test:semantic-invariants` 27, `test:check-pipeline-structure` 6. NOT run: `wrangler deploy --dry-run` (no credentials; host blocked by the egress proxy); the Linux `site-ci` run on the pushed head is the readiness authority. Rendered check with `astro dev` and headless Chromium: `/artistic-research/` 200 and both linked routes 200; the `### Public Surface Case` heading and both links render under Selected Public Reading; no horizontal overflow at 1280 px or 390 px; the Artistic Research page emits no robots meta (it stays indexable). Symbol hygiene: no literal ASCII not-equal marker in the touched Markdown; its non-ASCII characters are the em dashes the file already used.

Review provenance: unchanged - no independent review has been performed before this push; Reviewer interface none; lineage not applicable; mode not applicable; reviewed commit none; evidence reference none.

Pre-append inventory gate: `origin/main` unchanged (`6fc0f4ed`) and no branch or PR state change since the entries above; the inventory of the first 2026-09-19 entry stands. Gate does not stop.

Unresolved questions (for owner review, not blocking): (1) whether the 2026-08-22 case is to be listed, and with what genre sentence; (2) whether a series index page is wanted once the series grows; (3) the open items of the five entries above (Registry status of `Rapid Cultural Translation`; the compiled figures on the page; the wording of the conditions sentence; no PTS report; article bodies unread; rollover; no PR).

Risks or assumptions: this is the first link from an indexable page into the Public Surface Case series, so the series is now reachable without knowing a URL. Merge, PR and publication remain owner decisions and are not taken here.

### 2026-09-19 - Claude Code (Fable 5.1, Claude Code on the web / remote container) - claude/tender-turing-tniexe - 2026-09-19 page returned to the series register on owner ruling: compiled figures, vernacular motive gloss, societal-conditions sentence and the AI-compilation layer removed

Agent: Claude Code, model `claude-fable-5-1`, same container and session as the six entries above. Owner question: are the two pages (2026-08-18, 2026-09-19) consistent with MWE's overall register? Assessment given in the session: in method and form, yes (dated verify-only specimens, Observed / Research reading / Limit rubric, `≠` distinctions, candidate ≠ Registry, navigation-only relations, the field's own boundary statements as the most MWE-like movement); in register, four elements of the 2026-09-19 page pulled toward reportage or macro-sociology - (1) the `Scale, as compiled` row of second-hand figures, (2) the vernacular gloss "getting even on that party's behalf" in the Stated-motive row, (3) the "pluralistic society with open public discourse and democratic latitude" sentence, and (4) an AI-assisted research compilation as a cited source layer on a site whose governance holds that AI output is not authority. Owner ruling, verbatim: "1234都抽掉，讓他回到MWE，因為這個比較類似MWE在現實世界採樣，所以第一次這樣應該沒關係" - remove all four; the pair is accepted as a first real-world sampling at the outer edge of the site, with no further softening.

The change, all in `src/pages/artistic-research/public-surface-case/2026-09-19.astro` (+11 / -19 lines): the secondary-compilation layer is removed entirely - Movement 02 back to "three layers, now four" with the compilation line dropped from the layer list and its Limit row back to "vary between sources and are not adopted"; Movement 03's lead back to "dated by the screenshot that records it" and the compiled 2026-09-09 to 12 trajectory step removed; Movement 04's `Scale, as compiled` row removed, its `Scale` row back to one warm-up performance with "(figures not adopted)", the vernacular clause removed from `Stated motive`, the Research reading rewritten without the societal-conditions sentence and without the convergence passage ("This field adds a measure of how fast: a shared grievance on someone else's behalf, made sayable by a cultural template and made doable by a festival frame, gathered commitments from strangers in days, before any institution had spoken. The page records the speed and offers no account of what permits it. The speed is read as a property of this field, not of any people or place."), its Limit's "conditions are named, not measured" sentence removed, and the distinction `conditions named ≠ causes established` removed; the `As compiled` rows removed from Movements 05 and 06; the Limits block's compilation line removed and the evidence-count note back to "three institutional reports"; the Source snapshots lead and the `Compilation` row removed. Nothing derived from the compilation remains: a scan for `compil`, `pluralistic`, `getting even` and `as compiled` finds zero lines. What stays from the earlier rounds: Movements 04 and 05 themselves, the song layer described by subject with the singer unnamed, the owner-introduced `Rapid Cultural Translation` formulation, the official-versus-public reading in Movements 05 and 06, the CTS source, and the set-list row as author observation. The deep-research report remains reference material outside the repository; the owner's four decisions are recorded above.

Files changed (2): `src/pages/artistic-research/public-surface-case/2026-09-19.astro`, `AGENT_WORKLOG.md` (this entry, tail append only). `2026-08-18.astro` re-verified byte-identical to `origin/main`; `artistic-research.md` unchanged this round (its block still describes the pair accurately: "how fast cohesion formed and how the dispute's template was translated into an existing performance repertoire").

Owner ruling also recorded: the 2026-08-22 case is NOT to be linked from the Artistic Research page for now ("8/22先不用掛進去，那就先處理這兩篇就好"); no file change was needed, since the block added in the entry above links only the 2026-08-18 and 2026-09-19 pages.

Tests or build checks run (Linux container): `astro build` Complete; `check:astro` 0 errors / 0 warnings; `test:indexing-discovery` 234 pass / 0 fail / 1 skipped; `verify:indexing-discovery-build` 161/161; `verify:metadata-build` exit 0 (result line recorded in the commit message). Rendered check on the standalone preview at 1280 / 390 px: no horizontal overflow; seven preformatted blocks keep their line structure (7 / 3 / 4 / 3 / 4 / 4 / 8); seven source rows; six trajectory steps; twenty rubric rows. Symbol hygiene: no literal ASCII not-equal marker in the touched page; CJK text on exactly three lines, all news headlines. `node scripts/check-agent-worklog-governance.mjs` re-run after this append (result in the commit message).

Review provenance: unchanged - no independent review has been performed before this push; Reviewer interface none; lineage not applicable; mode not applicable; reviewed commit none; evidence reference none.

Pre-append inventory gate: `origin/main` unchanged (`6fc0f4ed`) and no branch or PR state change since the entries above; the inventory of the first 2026-09-19 entry stands. Gate does not stop.

Unresolved questions (for owner review, not blocking): (1) PR for this branch - not requested yet; (2) an independent review before merge; (3) the three open Dependabot PRs (#142, #144, #145) - the owner asked in the session that they be handled; that work needs a separate `sitegov/...-baseline-integration` branch by repository precedent and therefore the owner's explicit permission to push a branch other than this one, which had not been given at the time of this entry; (4) the open items of earlier entries (Registry status of `Rapid Cultural Translation`; no PTS report; article bodies unread; rollover).

Risks or assumptions: text-only removals inside one noindex page; no indexing, navigation, relation or candidate status affected. Merge, PR and publication remain owner decisions and are not taken here.

### 2026-09-19 - Claude Code (Fable 5.1, Claude Code on the web / remote container) - claude/tender-turing-tniexe - language-only change to the 2026-08-18 specimen on owner instruction; the 2026-09-19 page's "preserved unchanged" wording revised to "preserved as written"

Agent: Claude Code, model `claude-fable-5-1`, same container and session as the seven entries above. Owner instruction, verbatim: "0818的也稍微處理" - apply the same treatment to the 2026-08-18 page, lightly. Read as the language rule the owner set for the 2026-09-19 page ("只有新聞報導可以留中文": English throughout, news titles may stay in Chinese), applied at minimum scope and with no change to any statement. This is the first change to the 2026-08-18 page's text since it was published on 2026-08-18 (the 2026-08-22 change was markup only); every earlier entry of this session that says the page is byte-identical to `origin/main` was true when written and is not edited.

The change to `src/pages/artistic-research/public-surface-case/2026-08-18.astro` (+4 / -4 lines): the Movement 02 gloss `來源邊界（中文簡述）` is now `Source boundary (summary)` with its two paragraphs rendered in English (the reports confirm only that the controversy entered news circulation; the page does not undertake to prove that the proposed civic theatre or festivalization became a completed event; the object is how a public controversy enters cultural readability, platform remix and a possible performative civic surface; cultural casting is a public-reading mechanism, not a factual equivalence); and the duplicate Chinese sentence in the folded reconstruction note is removed, leaving its English sentence. NOT changed: the three reconstruction keywords (`鍘美藝術節`, `台南文藝負心運動`, `鍘美案 台南`) - they are functional search strings whose translation would not retrieve the field, so they are retained as the one exception beyond news titles, flagged for the owner; the five CNA / PTS headlines (news titles, retained per the rule); the `.access` note; every boundary statement, every rubric row, every distinction, every relation, every link. A character-class scan finds CJK text on exactly six lines: five headlines and the keyword line.

Consequential change to `src/pages/artistic-research/public-surface-case/2026-09-19.astro` (+6 / -6 lines): the page said in five places that the earlier specimen is "preserved unchanged" (meta description, subtitle, genre line, relations list, Source snapshots row); each now reads "preserved as written", which is what remains true. The folded Preservation note now says "This page does not revise the 2026-08-18 page" and records the language-only change and its date, stating that no statement was altered. The `artistic-research.md` block already said "preserved as written" and is not changed.

Files changed (3): the two case pages and `AGENT_WORKLOG.md` (this entry, tail append only).

Tests or build checks run (Linux container): `astro build` Complete; `check:astro` 0 errors / 0 warnings; `test:indexing-discovery` 234 pass / 0 fail / 1 skipped (the 2026-08-18 and 2026-09-19 tests both still pass: robots meta, exclusion, sibling contract); `verify:indexing-discovery-build` 161/161; `verify:metadata-build` exit 0, no FAIL line. Rendered check on standalone previews of both pages at 1280 / 390 px: no horizontal overflow; the 2026-08-18 page's five preformatted blocks keep their line structure (6 / 3 / 3 / 3 / 5) and its gloss heading renders as `Source boundary (summary)`; the 2026-09-19 page unchanged in layout (7 / 3 / 4 / 3 / 4 / 4 / 8). Symbol hygiene: no literal ASCII not-equal marker in either page. `node scripts/check-agent-worklog-governance.mjs` re-run after this append (result in the commit message).
...
### 2026-09-19 - Claude Code (Fable 5.1, Claude Code on the web / remote container) - sitegov/pr142-144-145-baseline-integration - owner-authorized baseline integration for Dependabot #142/#144/#145 (cloudflare-wrangler, dev-tooling, astro groups); adapter taken past #142's version to build against astro 7.3

Agent: Claude Code, model `claude-fable-5-1`, Claude Code on the web (isolated Linux container), working in a `git worktree` cut from `origin/main` (`6fc0f4ed593ff0d744d78408650ae44218d57f85`) so that the content branch `claude/tender-turing-tniexe` (PR #146) of the same session is untouched. Owner authorization in the session, verbatim: "順便把其他PR一起處理掉" (handle the other open PRs) followed by "都可以，延後 rollover，分支跟 PR 都開" (a separate branch and its PR may be opened). Owner explicitly reserved merge; this entry documents preparation to ready-for-review only.

Task: move guard 9's pinned dependency / lockfile baseline for the three open Dependabot PRs in one combined integration branch, per the #134/#135/#136 precedent (PR #138, entry of 2026-08-25 above). Dependabot's own branches cannot go green: guard 9 pins `package.json` and the lockfile bytes by design, so a bump is integrated only through an owner-authorized baseline move.

Finding that changed the scope: #145 (astro 7.2.4 -> 7.3.1, @astrojs/sitemap 3.7.3 -> 3.7.4) failed `site-ci` AND the Cloudflare Workers Build on a real build error, not on guard 9 - `[MISSING_EXPORT] "beginContentEntryCollection" is not exported by astro/dist/core/app/entrypoints/index.js`, imported by `@astrojs/cloudflare/dist/utils/prerender.js`. Confirmed in the container: astro 7.2.4 exports the symbol, 7.3.1 does not; adapter 14.2.5 (the version #142 proposes) and 14.2.6 both fail the same way against 7.3.1; 14.3.2 (npm latest at the time; peer ranges unchanged at astro ^7.2.0, wrangler ^4.125.0) is the first adapter that builds. The owner asked for the queue to be cleared, so the adapter is taken to 14.3.2 rather than integrating #142 + #144 alone and leaving #145 blocked. This is the one place this branch goes past what Dependabot proposed, and it is flagged in the PR body, in both guard-9 history comments, and here. Precedent for a forced extra step: the 2026-08-18 Astro 7 migration entry above (wrangler moved to 4.124.0 because the adapter's vite-plugin peered it).

Mechanics: `package.json` edited by exact-string substitution (five version strings; the file's formatting untouched); `pnpm install --lockfile-only` against the merged manifest, then a full `pnpm install` reproduced the identical lockfile (185,136 B, sha256 `6f2fb59a021756c5cde390e73ebe2dc1130a7d6ba644afb8de72e6bd2d4c3476`, 0 CR bytes), re-hashed before committing. Parsed lockfile package-NAME set (entries under `packages:` with the version suffix stripped) 516 -> 511: +2 `@astrojs/compiler-binding-android-arm64`, `find-proc`; -7 `chalk`, `color-convert`, `color-name`, `find-process`, `has-flag`, `loglevel`, `stream-replace-string`; 0 prohibited packages at any depth; no declared dependency added or removed (6 dependencies, 4 devDependencies). Transitive Cloudflare toolchain now workerd 1.20260826.1, miniflare 5.20260826.0-alpha, `@cloudflare/vite-plugin` 1.53.1, `@cloudflare/unenv-preset` 2.16.1. Because `@astrojs/sitemap` generates the public sitemap, the generated `dist/client/sitemap-0.xml` was compared with the main build's: identical apart from `lastmod`.

Guard 9 (`tests/public-surface-adjacency-map/renderingBoundary.test.ts`) moved in the SAME commit as the manifest and lockfile, per the rule stated in its own comment: `BASELINE_DEPENDENCIES` (`@astrojs/cloudflare` 14.2.3 -> 14.3.2, `@astrojs/sitemap` 3.7.3 -> 3.7.4, `astro` 7.2.4 -> 7.3.1), `BASELINE_DEV_DEPENDENCIES` (`wrangler` 4.125.0 -> 4.127.0, `fast-xml-parser` 5.11.0 -> 5.11.1), `LOCKFILE_IDENTITY` (183138 B `7e990329...ef5` -> 185136 B `6f2fb59a...3476`), with dated history comments in both comment blocks.

Files changed (3, commit `01a933f`): `package.json`, `pnpm-lock.yaml`, `tests/public-surface-adjacency-map/renderingBoundary.test.ts`; plus `AGENT_WORKLOG.md` (this entry, tail append only, in the following commit). No source, config, workflow, layout, style, page, or other test file changed. Pushed as branch `sitegov/pr142-144-145-baseline-integration`; PR #147 opened against `main`, NOT a draft, NOT merged.

Tests or build checks run (Linux container, on `01a933f`): guard 9 (`test:adjacency-rendering-boundary`) 13 pass / 0 fail; `astro build` Complete; `check:astro` 0 errors / 0 warnings; `check:ts` clean; every remaining step of `scripts.check` run individually in chain order with 0 failures - `test:metadata-contract` 27, `test:metadata-verifier-lifecycle` 5, `test:contracts` 52, `test:authority-layout` 68, `test:authority-viewport` 61, `test:authority-keyboard` 38, `test:runtime` 55, `test:retention` 16, `test:orchestration` 29, `test:workflow` 42, `test:semantic-flow` 21, `test:security-resilience` 128, `test:indexing-discovery` 233 pass / 1 skipped, `verify:public-surface-map` 21/21, `verify:indexing-discovery-build` 161/161, `verify:metadata-build` exit 0, `test:human-governed` 30, the eleven adjacency suites (58 / 18 / 23 / 39 / 10 / 10 / 8 / 13 / 56 / 22 / 60), `verify:public-surface-adjacency-map` 21/21, `test:html-charset` 16, `test:semantic-invariants` 27, `test:check-pipeline-structure` 6. NOT run: `wrangler deploy --dry-run` (no Cloudflare credentials in the container; the Cloudflare host is blocked by its egress proxy). The Linux `site-ci` run and the Workers Build on PR #147's head are the readiness authority. `node scripts/check-agent-worklog-governance.mjs` re-run after this append (result in the commit message).

Review provenance: no independent review has been performed before this push; Reviewer interface none; lineage not applicable; mode not applicable; reviewed commit none; evidence reference none. The owner may request a Codex or Copilot review on the pushed head.

Pre-append inventory gate, run at write time: `git fetch origin --prune`; `origin/main` = `6fc0f4ed` (unchanged since the 2026-08-26 entries above). PR state obtained through the GitHub MCP tools (the governance script reports `pr_state_unavailable (gh_absent)`). Open PRs: the three Dependabot PRs #142 / #144 / #145 (the subject of this task, listed as the dependency queue), #146 (`claude/tender-turing-tniexe`, this session's content branch, owner-authorized, `in_progress` pending owner merge), and #147 (this branch). Remote non-bot branches not ancestors of `origin/main`: the six standing branches under the 2026-08-06 / 08-15 / 08-17 owner rulings and the two same-stream P7 branches, unchanged and not blocking; plus `claude/tender-turing-tniexe` (author status: owner-authorized, PR #146 open). No branch classified `completed_pushed_unmerged`, `ambiguous`, or `author_status_unknown`. The `origin/main` worklog was verified as an exact byte prefix of this file before and after this append. Rollover: this branch's worklog is 4,838 lines from `main` plus this entry; the owner's 2026-09-19 deferral ruling (recorded on PR #146's branch) covers the content branch, which is past 5,000 lines. Gate does not stop. The inventory is advisory only and authorizes no merge, publication, or deployment.

Unresolved questions (for owner review, not blocking): (1) whether taking `@astrojs/cloudflare` to 14.3.2 instead of #142's 14.2.5 is accepted - if not, the alternative is a #142 + #144-only integration with #145 left blocked until a 14.2.x adapter that builds against astro 7.3 exists; (2) #142, #144 and #145 are to be closed by the owner after this merges (or will be superseded by Dependabot's next run); (3) if #146 merges first, this branch and #146 touch disjoint files (no `AGENT_WORKLOG.md` conflict is expected because both append to the same `main` tail, but whichever lands second must tail-re-append its worklog entry onto the landed `main` worklog and re-run the byte-prefix check, per the 2026-08-26 precedent above); (4) astro 7.3.3 and adapter versions newer than those taken here exist on npm; Dependabot will propose them in its next run.

Risks or assumptions: the lockfile was regenerated rather than hand-merged and the regenerated bytes were verified stable across a second, full install; the authoritative confirmation is the Linux `site-ci` run and the Workers Build on PR #147. The adapter minor bump (14.2 -> 14.3) is within the same major and the same declared peer ranges; its changelog was not read from this container (egress blocked) beyond the release-notes excerpt in #142's body. Merge is owner-reserved and NOT taken here.

### 2026-09-19 - Claude Code (Fable 5.1, Claude Code on the web / remote container) - sitegov/pr142-144-145-baseline-integration - stacked on PR #146 at owner request: content branch merged in, worklog tail composed by bytes, PR #147 re-based onto claude/tender-turing-tniexe

Agent: Claude Code, model `claude-fable-5-1`, same container and session as the entries above. Owner instruction, verbatim: "前面的你不能一起修嗎" - asked, after being told that whichever of #146 / #147 landed second would need its worklog tail re-appended, whether that could be fixed up front. It can, by stacking: PR #147 now contains PR #146's commits, so the owner merges #146 first and #147 second with no intermediate step.

The change: merge commit `9a77723` (`git merge --no-ff origin/claude/tender-turing-tniexe` at `42daab1`; no history rewritten on either branch). The only overlapping file, `AGENT_WORKLOG.md`, conflicted because both branches append at the same tail; it was rebuilt by a script from committed bytes, not by hand-editing conflict markers: `origin/main`'s file (766,253 B, 4,837 LF) followed byte-for-byte by #146's appended entries (56,231 B, 189 LF) followed byte-for-byte by #147's appended entry (8,684 B, 24 LF) -> 831,168 B, 5,050 LF, 0 CR bytes; `origin/main`'s file verified as an exact byte prefix of both sides before composing and of the result after writing. Every other file merged cleanly: the two branches touch disjoint files (#146: the two case pages, `artistic-research.md`, `indexing-discovery-contract.mjs`, `indexing-discovery.test.ts`; #147: `package.json`, `pnpm-lock.yaml`, `renderingBoundary.test.ts`). PR #147's base is changed from `main` to `claude/tender-turing-tniexe`, so its diff shows only the dependency integration; when #146 merges, GitHub retargets #147 to `main` (or the agent does on the event).

Files changed: none beyond the merge itself and this entry (`AGENT_WORKLOG.md`, tail append only).

Tests or build checks run on `9a77723` (Linux container): `node scripts/check-agent-worklog-governance.mjs` `Append-only (byte prefix at observed integration commit 6fc0f4ed): true`, 5,051 lines (over 5,000 under the owner's 2026-09-19 rollover deferral, which this entry extends to this branch); guard 9 13 pass / 0 fail; `astro build` Complete; `check:astro` 0 errors / 0 warnings; `check:ts` clean; `test:indexing-discovery` 234 pass / 0 fail / 1 skipped (both case-page tests pass against the merged tree); `test:adjacency-preservation` 8/8; `verify:indexing-discovery-build` 161/161; `verify:metadata-build` exit 0; `test:check-pipeline-structure` 6/6. The full chain passed on each side separately (entries above); the `site-ci` run and Workers Build on the pushed head are the readiness authority for the merged tree.

Review provenance: unchanged - no independent review; Reviewer interface none; lineage not applicable; mode not applicable; reviewed commit none; evidence reference none.

Pre-append inventory gate: `origin/main` unchanged (`6fc0f4ed`); #146 and #147 open, both green before this merge; no other change. Gate does not stop.

Unresolved questions (for owner review, not blocking): (1) merge order is now fixed: #146 first, then #147; merging #147 first would also bring #146's content in, which is not the intent; (2) items of the entry above unchanged (adapter 14.3.2 past #142; closing #142 / #144 / #145 after merge).

Risks or assumptions: the stacking couples the dependency integration to the content PR's landing; if #146 is rejected, this branch must be rebuilt from `main` without it (the three integration files are untouched by #146, so that is a re-cut, not a re-do). Merge is owner-reserved and NOT taken here.
