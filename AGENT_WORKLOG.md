...
### 2026-07-27 — Claude Code — p7-2-commit-a-pure-viewport-foundation

Agent: Claude Code
Task: P7.2 Commit A — the pure foundation of the approved REV7 plan. Add the pure viewport module and its 41 canonical registrations, and wire the new suite into `check`. No control is rendered, no listener is installed and no forward gate is opened in this commit.
Files changed: src/lib/public-surface-adjacency-map/viewport.ts (new); tests/public-surface-adjacency-map/viewport.test.ts (new); package.json (scripts only — `test:adjacency-viewport` added and appended to `check`); AGENT_WORKLOG.md.
Build / tests run: `node --test` per adjacency suite — viewport 41/41 (new), radial-layout 56/56, directional-navigation 22/22, visual-state 60/60, rendering-boundary 13/13, interaction 39/39 (exact count preserved), preservation 8/8, metadata 10/10, contract 58/58, runtime-manifest 18/18, runtime 23/23, endpoints 10/10. All 0 failed, 0 skipped. `npx tsc --noEmit` clean.
Result: Canonical checks 155–181, 182, 184, 200–205 and 207–212 are implemented and passing against the real production module. `viewport.ts` enters the guard-13 production scan scope and passes guards 1–13 unchanged. `reducePointer` is defined as the sole pure pointer-transition authority; it is not yet wired, because wiring belongs to atomic Commit B. `preservation.test.ts` stays green because its pipeline assertion is a prefix check and the new script is appended.
Unresolved questions: None.
Risks or assumptions: Implementation baseline is frozen at 5271c03c6f6e2cc7624fcd283c006dad02b56c0f per owner ruling; later remote-main movement is not tracked. Dependencies, devDependencies, packageManager and pnpm-lock.yaml are untouched. No P7.2 control, handler, placeholder or forward-gate re-scope exists in this commit.

### 2026-07-27 — Claude Code — p7-2-commit-b-atomic-implementation

Agent: Claude Code
Task: P7.2 Commit B — the atomic implementation of the approved REV7 plan: five viewport controls, arc activation, the pointer and wheel lifecycle, the shortcut path, the tooltip, and every forward-gate re-scope, all in one commit.
Files changed: src/lib/public-surface-adjacency-map/publicWording.ts (five approved label constants, add-only); src/components/PublicSurfaceAdjacencyMap.astro (five toolbar buttons, Focus Record native `disabled`, arc control semantics + `data-psadj-arc-action`, tooltip element, `vector-effect: non-scaling-stroke`, authored identity transform on the viewport wrapper); src/scripts/public-surface-adjacency-map.ts (radial/viewport/pointer state, `pointerTargetKind`, wheel with `{ passive: false }`, the full pointer table routed through `reducePointer`, `finalizePointer` with `PointerTerminalKind`, one-shot drag suppression, `resolveGroupFitInput`, arc activation, corrected runtime activation, shortcut-first keydown); tests/public-surface-adjacency-map/visualState.test.ts (+23 canonical registrations; gates 86/105/106/107/108/129 re-scoped); tests/public-surface-adjacency-map/directionalNavigation.test.ts (+4 canonical registrations 213–216; gate 77 re-scoped); tests/public-surface-adjacency-map/renderingBoundary.test.ts (gate 6 — P7.3 token list replaced with a non-emptiable 11-token set, plus the new production-source path-and-content scan folded into guard 13); scripts/verify-public-surface-adjacency-map-build.mjs (PSADJ-18 and PSADJ-20 re-scoped, PSADJ-22/23/24 added, EXPECTED_PSADJ_CHECKS 21 -> 24); AGENT_WORKLOG.md.
Build / tests run: viewport 41/41, radial-layout 56/56, directional-navigation 26/26, visual-state 83/83, rendering-boundary 13/13, interaction 39/39 (exact count preserved, file UNMODIFIED), preservation 8/8, metadata 10/10, contract 58/58, runtime-manifest 18/18, runtime 23/23, endpoints 10/10; authority-layout 68/68, authority-viewport 61/61, authority-keyboard 38/38; `npx tsc --noEmit` clean; `npx astro check` 0 errors; `astro build` succeeds; `verify:public-surface-adjacency-map` 24/24 with the identifier sequence PSADJ-01…PSADJ-24. All 0 failed, 0 skipped.
Result: Canonical checks 183, 185–199, 206, 213–216 and 217–222 are implemented and passing. Allocation is 41 / 23 / 4 = 68 with three verifier additions owning zero canonical IDs. The contingency file `interaction.test.ts` was NOT modified: its named assertion was briefly falsified by a `visibility: hidden` CSS declaration, which was changed to `opacity: 0`, restoring 39/39 with the file byte-identical.
Unresolved questions: **`pnpm run check` FAILS at step 18** — `verify-public-surface-map-build.mjs` check 20 clause (e) forbids `addEventListener("wheel" | "pointerdown" | "pointermove")` across EVERY `dist/_astro/*.js` via `clientJsFiles()`, so the adjacency bundle trips a guard whose stated subject is the authority map (`data-psam-*`, Phase 2B-1). The authority map's own bundle still carries 0 gesture markers, so the guard's real contract is intact; the scan is over-scoped, exactly as PSADJ-21 was before the P7.1 P7-01 correction. That file is on the REV7 §15.3 FROZEN list, so correcting its scope is a frozen-path change and is NOT made here. Reported for owner ruling under approval condition 9.
Risks or assumptions: Baseline frozen at 5271c03c6f6e2cc7624fcd283c006dad02b56c0f. No dependency, devDependency, packageManager or pnpm-lock.yaml change. P7.3 remains excluded and its forward boundary is now enforced over both the two named test files and all 92 production entries by path and content. Arc pointer-target precision at scale 1.0 is a real-device question left to preview evidence D16; no hit-widening element was invented, because the approved plan specifies none.

### 2026-07-27 — Claude Code — p7-2-commit-c-frozen-verifier-scope-correction

Agent: Claude Code
Task: P7.2 Commit C — the owner-authorized bounded correction to `verify-public-surface-map-build.mjs` check 20 clause (e), whose gesture-listener scan covered every emitted bundle rather than the Authority Map's own, plus the two consequential byte-identity records that pin that verifier.
Files changed: scripts/verify-public-surface-map-build.mjs (check 20 clause (e) only — scoped to the Authority Map's own bundles by a positive `data-psam-viewport` / `psam__layer--viewport` predicate, with an empty-set rejection and in-verifier positive and negative controls; clauses (a)–(d) and (f) unchanged in effect, and no gesture prohibition weakened, removed, bypassed or renamed); tests/public-surface-adjacency-map/preservation.test.ts (exactly one FROZEN_IDENTITIES entry — byteLength 31519 → 33920, SHA-256 0a248c87… → 8f87c5a6…, gitBlob de691294… → 22ec8d5a…; 20 entries and every assertion unchanged); scripts/verify-public-surface-adjacency-map-build.mjs (exactly one line — the PSADJ-14 frozen-product Git blob for the same file; 7 entries unchanged); AGENT_WORKLOG.md.
Build / tests run: `pnpm run check` **exit 0**, 0 failing lines. verify:public-surface-map 21/21; verify:public-surface-adjacency-map 24/24 with sequence PSADJ-01…PSADJ-24. viewport 41/41, radial-layout 56/56, directional-navigation 26/26, visual-state 83/83, rendering-boundary 13/13, interaction 39/39, preservation 8/8, metadata 10/10, contract 58/58, runtime-manifest 18/18, runtime 23/23, endpoints 10/10; authority-layout 68/68, authority-viewport 61/61, authority-keyboard 38/38. `npx tsc --noEmit` clean; `npx astro check` 0 errors 0 warnings; `astro build` succeeds; `git diff --check` clean.
