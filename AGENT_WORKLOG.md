...
## 2026-08-17 - link a related candidate governance surface from two pages

Agent used: Claude Code (Opus), PC. Branch `claude/related-governance-surface`, cut from `origin/main` `01ee5c17783455d5420fd544edd2f8295c984eca`.

Task performed: added a discoverable but deliberately secondary link to one independently published external repository, `metawritingecology/lineage-aware-agent-governance`, on two existing pages. This is two thirds of a reciprocal cross-link; the corpus repository and the governance repository each gain their own link in separate pull requests of their own.

Files changed: `src/pages/surfaces.md` (8 lines added, 0 deleted), `src/pages/human-governed-ai-workflows.astro` (10 lines added, 0 deleted), `AGENT_WORKLOG.md` (this entry). The whole change set is 18 added lines and **zero deleted lines** across both content files, so no existing boundary statement, category heading, measurement state, or inference ceiling was altered by construction.

`/surfaces/` - new `## Related / candidate public surfaces` section placed AFTER `## Boundary note` and before `## Related boundary pages`. The placement is load-bearing, not incidental: the page defines four surface categories as `##` headings and `## Boundary note` then enumerates them by name ("The fiction, system, corpus, and platform surfaces should not be collapsed into one format"). A new `##` section placed before that sentence becomes a fifth item in a list the sentence then fails to enumerate, which reads either as an omission or as a fifth confirmed category. Placed after it, the four-category enumeration stays closed. The heading also uses the page's sentence case and deliberately avoids the `<Name> surface` pattern the four categories use. Verified on the rendered page: `h2` order is Fiction surface, System surface, Corpus surface, Platform surface, Boundary note, Related / candidate public surfaces, Related boundary pages - the new heading is seventh, after the boundary note, and the four category headings are unchanged and still four.

`/human-governed-ai-workflows/` - one `<p>` appended to the end of the `Multi-Agent Execution and Retained Answerability` section, after the existing `Direct evidence.` paragraph. That section is the closest subject match on the site: its `Declared controls` paragraph already describes bounded task scope, role separation, review requirements, retained answerability, and final user authority. The same section's `Direct evidence.` rule forbids linking "internal routing logic" while permitting "source documents that declare the boundaries"; the linked document declares boundaries in abstract terms and states that it excludes complete internal policy, live routing state, authorization structure, credentials, operator process, exact thresholds, and internal routing details, so it falls on the permitted side. That classification was independently reviewed against the full linked document before this change was made. The added paragraph is worded as adjacency and explicitly denies being evidence - it states it does not change the measurement state above, does not extend the inference ceiling stated above, and does not establish MWE component status, Registry status, or formal dependency - so it cannot be read as raising the section's `Not measured` state or weakening its inference ceiling.

Relation status, stated plainly: the linked repository is a candidate / related public surface and is NOT a confirmed MWE component. Publishing links does not establish Registry status, formal ontology, confirmed conceptual relation, dependency, component membership, model status, inheritance, priority, authority, or canonical interpretation. Both new passages say so in their own text. `/surfaces/` links this repository's `RELATION_STATUS_GUIDE.md` so the term `candidate relation` is fixed by an already-published definition rather than by new wording invented here.

Not changed, deliberately: global navigation and the homepage (top-navigation inclusion is a decision `AGENTS.md` states agents must not make, and is owner-reserved); `/public-surface-map/` and both its 30-record and 59-record interactive views, which are record-counted datasets under byte-identity contracts and would have had their counts changed; `/atlas/`, `/models/`, and every relation or model structure; canonical URLs, sitemap, redirects, and all indexing behaviour; JSON-LD; and `schema.org` `sameAs`, which was considered and rejected because a candidate relation is not an identity claim. No governance text was duplicated onto the site; each page carries one hyperlink.

Checks run. Pre-append inventory gate, run before this write, current branch excluded. `git fetch origin`; `origin/main` is `01ee5c17`; PR #124 (`cursor/site-entry-doors`) is MERGED as of 2026-08-16T00:28:18Z. `node scripts/check-agent-worklog-governance.mjs` exit 0. Five branches reported `requires_author_or_pr_review=true` and each was checked against the recorded author ruling in this file rather than against a previous round's summary of it: `claude/p7-1-implementation-plan-7t42ah` (#101 CLOSED), `codex/update-site-from-meta-writing-ecology` (#10 CLOSED) and `fix-public-surface-metadata-and-crawler-files` (#1 CLOSED) carry the author's 2026-08-15 ruling recorded above that they continue separately; `rev10-deployment-metadata` (no PR metadata) was classified `hold` by the author on 2026-08-07; `chore/update-download-artifact-20260801` (#106 CLOSED) was classified `superseded` by the author on 2026-08-06. None is `completed_pushed_unmerged`, `ambiguous`, or `author_status_unknown` lacking author status, so the gate does not stop. File overlap, recorded and NOT integrated: `codex/update-site-from-meta-writing-ecology` (tip `8b3ab85`, +2 lines) and `fix-public-surface-metadata-and-crawler-files` (tip `729455b`, +28 lines) both touch `src/pages/surfaces.md`, which is this change set's target. Both are author-declared continue-separately with CLOSED pull requests; no content was taken from either, and if either is revived it will conflict on that file and must be reconciled by the author then. Dependency queue, listed separately and not treated as feature work: `dependabot/npm_and_yarn/cloudflare-wrangler-9af8a2a62a`, open PR #126, wrangler 4.121.0 to 4.123.0, not touched.

Build and render checks: `pnpm install --frozen-lockfile` exit 0; `pnpm run build` exit 0, sitemap created, server built. The build target is `output: "server"` with the Cloudflare adapter, so neither changed page is emitted as static HTML and neither could be verified from `dist/`; `astro preview` requires the adapter's Cloudflare runtime and failed locally without credentials. Both pages were therefore fetched from a local `astro dev` server and checked on the rendered HTML: `/surfaces/` renders the new `h2` in the position described above with exactly one governance-repo link and one Relation Status Guide link; `/human-governed-ai-workflows/` renders the new paragraph with exactly one governance-repo link; both pages carry the expected apex-HTTPS canonical (`https://metawritingecology.org/surfaces/` and `.../human-governed-ai-workflows/`); zero occurrences of `www.metawritingecology` on either page or anywhere in `dist/`; the generated sitemap contains only the apex origin and does not contain the external GitHub URL. Symbol hygiene: 0 literal `!=` in any added line. `pnpm run check` was not run end to end because it includes `wrangler deploy --dry-run`, which needs Cloudflare credentials; CI is authoritative.

Rollover: this file was 4,477 lines and 642,510 bytes before this append, status `review_threshold_reached` - past the 4,000-line review trigger and under the 5,000-line ceiling. One further append is in bounds. Rollover is a separate authorized task and was NOT executed. The reconfirmation outstanding in previous entries remains outstanding and is flagged to the author again here.

Unresolved questions: None blocking. Whether the two continue-separately branches that touch `src/pages/surfaces.md` are revived, and how they would then be reconciled with this section, is an author decision. Whether the linked repository is ever promoted from a candidate relation to a confirmed one is an owner decision under the applicable MWE authority process and was not taken here.

Risks or assumptions: The plan behind this change received one independent review from a distinct model lineage, verdict `ACCEPT_WITH_FINDINGS`, with every finding re-checked on the originating machine before execution. The corrected plan that this change implements was not itself independently reviewed; its corrections were mechanical rather than conceptual. The judgement that the linked document is a boundary-declaring source rather than internal routing logic is a judgement, not a mechanical determination, although it survived that review against the full linked document. No test asserts that the boundary wording stays attached to either link, so a later editor could delete one and leave the other. No merge, no PR close, no navigation authority decision, and no relation promotion was made here.
